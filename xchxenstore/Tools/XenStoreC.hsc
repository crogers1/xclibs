...
module Tools.XenStoreC (
                         xsRead, xsReadH
                       , xsWrite, xsWriteH
                       , xsWriteUTF8, xsWriteUTF8H
                       , xsRm, xsRmH
                       , xsDir, xsDirH
                       , xsWaitFor, xsWaitForH, xsWaitFor', xsWaitForH'
                       , Permission(..)
                       , PermissionFlag(..)
...
withUTF8CStringLen s f = do
    let bytes = (B.unpack . UTF8.fromString $ s) ++ [0]
    withArrayLen bytes $ \l aptr -> let cs = castPtr aptr in f (cs,l-1)
...
xsWriteUTF8H :: Xs -> String -> String -> IO ()
xsWriteUTF8H h path value =
    withCString path $ \pathS ->
        withUTF8CStringLen value $ \(valueS,valueLen) ->
            do ret <- c_xs_write (handle h) (fromIntegral 0) pathS valueS (fromIntegral $ valueLen)
               return ()
    ...
    withCString path $ \pathS ->
        withCString token $ \tokenS ->
            do lock <- newEmptyMVar
               ...
               ret <- c_xs_watch (handle h) pathS tokenS
               return ()
...
xsWaitForH' :: Xs -> String -> IO (Maybe a) -> IO a
xsWaitForH' h path pred =
    do xsWatchH h path path
       debug ("xs -> watching for " ++ show path)
       watch `E.catch` (\(e::E.SomeException) -> xsUnwatchH h path path >> E.throw e)
    where
       watch = do
         m <- readMVar (watches h)
         -- path used as token
         ...
         p <- pred
         case p of
           Just x -> do debug ("xs -> watch " ++ path ++ " predicate tested positive")
                        xsUnwatchH h path path
                        return x
           _    ->   do debug ("xs -> watch " ++ path ++ " predicate tested negative, continuing..")
                        watch
...
xsWriteUTF8 :: String -> String -> IO ()
xsWriteUTF8 p v = withXs $ \h -> xsWriteUTF8H h p v
...
xsWaitFor' :: String -> IO (Maybe a) -> IO a
xsWaitFor' p pred = withXs $ \h -> xsWaitForH' h p pred
